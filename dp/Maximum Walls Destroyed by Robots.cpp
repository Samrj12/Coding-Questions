...
public:
    int maxWalls(vector<int>& robots, vector<int>& d, vector<int>& walls) {
        ...
        for (int i = 1; i < n; i++) {
            dp[i][0] = dp[i - 1][0] + query(max(x[i - 1][0] + 1, x[i][0] - x[i][1]), x[i][0]);
            int overLap = query(max(x[i - 1][0] + 1,x[i][0] - x[i][1]), min(x[i][0] - 1, x[i - 1][0] + x[i - 1][1]));
            dp[i][0] =
                max(dp[i][0],
                    dp[i - 1][1] + query(max(x[i - 1][0] + 1,x[i][0] - x[i][1]), x[i][0]) - overLap);
